...
export default function Contact() {
  ...
  function submit(e) {
    e.preventDefault();
    fireToast('Message envoyé — nous répondons sous 48 h.');
    e.target.reset();
  }
    ...
    <section className="ttt-section" style={{ paddingTop: 60, paddingBottom: 80 }}>
      <div className="ttt-cols-contact">
        ...
        <form onSubmit={submit} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 30, borderRadius: 20 }}>
          ...
          <Select label="Motif" defaultValue="Question sur le jeu">
            <option>Question sur le jeu</option>
            <option>Problème avec un code</option>
            <option>Récupération d'un lot</option>
            <option>Données personnelles</option>
            <option>Autre</option>
          </Select>
          <Textarea label="Message" required rows={4} placeholder="Votre message…" />
